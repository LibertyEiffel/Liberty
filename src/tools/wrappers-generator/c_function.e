class C_FUNCTION
   -- A "Function" node of an XML file made by castxml.
...
feature {ANY}
   store
      do
         log.info.put_line("Storing function '#(1)', line #(2)" # c_string_name # line_row.to_utf8 )
         if is_anonymous then
               log.info.put_line("Skipping anonymous funtion at line #(2)" # line_row.as_utf8)
         else
            check
                 c_string_name /= Void
            end
   ...
   is_wrappable: BOOLEAN
      ...
      do
         ...
         Result := a_return_type/=Void and then
            ( a_return_type.is_void or
            a_return_type.has_wrapper)
         if Result then
            -- Check for
            from
               i := children_count
   ...
   wrap_on (a_stream: OUTPUT_STREAM)
      do
         if not is_wrappable then
            log.info.put_line("Function `#(1)' is not wrappable" #  c_string_name)
            buffer.reset
         ...
         elseif not is_public then
            log.info.put_line(once "Skipping 'hidden' function `#(1)'" # c_string_name)
            buffer.append(once "%T-- `hidden' function #(1) skipped.%N" # c_string_name)
         elseif not namespace.is_main then
            log.info.put_line(once "Skipping function `#(1)' belonging to namespace #(2) which is not wrapped" #
         ...
         elseif avoided_symbols.has(c_string_name) then
             log.info.put_line(once "Function `#(1)' is among the avoided symbols: not wrapping as requested." # c_string_name)
            buffer.append(once "%T-- function #(1) skipped as requested.%N" # c_string_name)
         else
            log.info.put_line(once "Function #(1)" # c_string_name)
            buffer.append(once "%T#(1)" # eiffel_name)
			append_arguments
            append_return_type
            append_description
            append_body
            log.info.put_line(once "%N")
         end
   ...
   append_description
         ...
         local description: COLLECTION[STRING];  word: STRING; iter: ITERATOR[STRING];
   ...
   append_arguments
      ...
      do
         if has_arguments then
            -- too verbose log.info.put_line(once " (")
            buffer.append(once " (")
            ...
            -- too verbose log.info.put_line(once "#(1) args: " # &children_count)
            from
               i := 1
            until
               i > last - 1
   ...
   append_return_type
         ...
         -- Current node to `buffer' i.e. ": INTEGER_32 " or ": POINTER is".
         -- When result of `a_node' is "void" nothing.
      do
         ...
         log.info.put_line(once "Unwrappable return type: #(1)... " # developer_exception_name)
      end
