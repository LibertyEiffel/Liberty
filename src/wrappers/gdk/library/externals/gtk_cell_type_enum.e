-- This file have been created by wrapper-generator.
-- Any change will be lost by the next execution of the tool.

expanded class GTK_CELL_TYPE_ENUM

-- TODO emit_description(class_descriptions.reference_at(an_enum_name))

insert ENUM

creation default_create
feature -- Validity
	is_valid_value (a_value: INTEGER): BOOLEAN is
		do
			Result := ((a_value = empty_low_level)  or else
				(a_value = text_low_level)  or else
				(a_value = pixmap_low_level)  or else
				(a_value = pixtext_low_level)  or else
				(a_value = widget_low_level) )
		end

feature -- Setters
	default_create,
	set_empty is
		do
			value := empty_low_level
		end

	set_text is
		do
			value := text_low_level
		end

	set_pixmap is
		do
			value := pixmap_low_level
		end

	set_pixtext is
		do
			value := pixtext_low_level
		end

	set_widget is
		do
			value := widget_low_level
		end

feature -- Queries
	is_empty: BOOLEAN is
		do
			Result := (value=empty_low_level)
		end

	is_text: BOOLEAN is
		do
			Result := (value=text_low_level)
		end

	is_pixmap: BOOLEAN is
		do
			Result := (value=pixmap_low_level)
		end

	is_pixtext: BOOLEAN is
		do
			Result := (value=pixtext_low_level)
		end

	is_widget: BOOLEAN is
		do
			Result := (value=widget_low_level)
		end

feature {WRAPPER, WRAPPER_HANDLER} -- Low level values
	empty_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module_name: "plugin"
 			feature_name: "GTK_CELL_EMPTY"
 			}"
 		end

	text_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module_name: "plugin"
 			feature_name: "GTK_CELL_TEXT"
 			}"
 		end

	pixmap_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module_name: "plugin"
 			feature_name: "GTK_CELL_PIXMAP"
 			}"
 		end

	pixtext_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module_name: "plugin"
 			feature_name: "GTK_CELL_PIXTEXT"
 			}"
 		end

	widget_low_level: INTEGER is
		external "plug_in"
 		alias "{
 			location: "."
 			module_name: "plugin"
 			feature_name: "GTK_CELL_WIDGET"
 			}"
 		end


end -- class GTK_CELL_TYPE_ENUM
