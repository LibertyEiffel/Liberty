...
feature {ANY}
	...
	first: STRING is 
		do
			create Result.from_external_copy (gslist_struct_get_data (handle))
		end

	last: like first is 
		do
			create Result.from_external_copy (gslist_struct_get_data (g_slist_last (handle)))
		end
	...
	put (a_string: like first; i: INTEGER) is
		...
		do
			gslist_struct_set_data (g_slist_nth(handle,i.to_natural_32), a_string.to_external)
		end

	swap (i,j: INTEGER) is
		local ith,jth,tmp: POINTER
		do
			ith := g_slist_nth_data (handle,i.to_natural_32)
			jth := g_slist_nth_data (handle,j.to_natural_32)
	...
	set_all_with (v: like first) is
		...
		do
			...
			until ith.is_null
			loop
				gslist_struct_set_data (ith, v.to_external)
