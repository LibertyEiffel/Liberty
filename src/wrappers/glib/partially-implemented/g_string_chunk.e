...
class G_STRING_CHUNK
	-- An efficient storage of groups of strings.
	
	-- String chunks are used to store groups of strings. Memory is
	-- allocated in blocks, and as strings are added to the
	-- G_STRING_CHUNK they are copied into the next free position in a
	...
	-- To add strings to a G_STRING_CHUNK use g_string_chunk_insert().
	
	-- To add strings to a G_STRING_CHUNK, but without duplicating
	-- strings which are already in the G_STRING_CHUNK, use
...
feature {ANY}
	...
	-- Adds a copy of string to the G_STRING_CHUNK. It returns a pointer to the new copy of the string in the G_STRING_CHUNK. The characters in the string can be changed, if necessary, though you should not change anything after the end of the string.
	...
	-- Adds a copy of string to the G_STRING_CHUNK, unless the same string has already been added to the G_STRING_CHUNK with g_string_chunk_insert_const().
