...
inherit
   ...
   G_FREEZABLE
		-- Some GLib using libraries requires that some instances of GList
		-- shall be modified only by the library, making it effectively freezed
		-- for the developer. These shall return a G_LIST_TRAVERSABLE. 
		-- If the library allows , that the list is modified by Eiffel 
