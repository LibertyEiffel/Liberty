...
feature {} -- Creation
	...
	new_file (a_file_name, a_mode: STRING) is
		-- Open a file named `a_file_name' as a GIOChannel using `a_mode'. This channel
		-- will be closed when the last reference to it is dropped, so there is
		-- no need to call `close' (though doing so will not cause
		-- problems, as long as no attempt is made to access the channel after
		-- it is closed).

		-- mode shall be one of "r", "w", "a", "r+", "w+", "a+". These have the
		-- same meaning as in fopen().

		-- `error' will be updated.
	local p: POINTER
	do
		p := g_io_channel_new_file
		(a_file_name.to_external, a_mode.to_external, error.reference)
...
feature {ANY} -- Access
	...
	do
		Result:=g_io_channel_get_buffered(handle).to_boolean
	end
	...
	last_failed: BOOLEAN is
		...
		do
			Result := not last_status.is_normal
		end
...
feature {ANY} -- Operations
	...
	add_watch (condition: GIOCONDITION_ENUM; action: FUNCTION [TUPLE [G_IO_CHANNEL, INTEGER], BOOLEAN]) is
		...
		do
			...
			last_event_source := g_io_add_watch (handle, condition.value, callback.function, callback.data)
		end
...
feature {ANY} -- Queries
	...
	do
		Result := g_io_channel_unix_get_fd (handle)
	end
...
-- g_io_channel_write_chars ()
	...
	write_chars (chars: ABSTRACT_STRING) is
		...
		do
			...
			debug 
				print (once "failed=") print(last_failed.to_string) print(once "%N") 
			end
		end
