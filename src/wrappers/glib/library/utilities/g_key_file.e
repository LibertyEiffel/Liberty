	...
	load_from_file (a_file: STRING; some_flags: GKEY_FILE_FLAGS_ENUM) is
		...
		do
			...
			is_successful:=(g_key_file_load_from_file
								 (handle, a_file.to_external, some_flags.value,
								  error.reference)).to_boolean
		ensure meaningful_error: -- TODO
		end
...
feature {ANY} -- Queries
	...
	strings, string_list (a_group, a_key: STRING): COLLECTION[STRING] is
...
feature {ANY} -- Setting commands
	...
	set_integers (a_group, a_key: STRING; some_integers: COLLECTION[INTEGER]) is
			...
			-- Performance hint: if `some_integers' is an heir of
			-- ARRAYED_COLLECTION no temporary copies of given data are made.
			-- Otherwise there is an O(some_integers.count) overhead both in
			-- memory and time.
		require
			key_not_void: a_key/=Void
			valid_integers: (some_integers/=Void and then 
								  not some_integers.is_empty)
		local array: ARRAYED_COLLECTION[INTEGER_32]
		do
			array ?= some_integers
			if array=Void then 
				create {FAST_ARRAY[INTEGER_32]} array.from_collection (some_integers)
			end
	...
	set_reals (a_group, a_key: STRING; some_reals: TRAVERSABLE[REAL]) is
			...
			-- Performance hint: if `some_reals' is an heir of
			-- ARRAYED_COLLECTION no temporary copies of given data are made.
			-- Otherwise there is an O(some_integers.count) overhead both in
			-- memory and time.
		...
		local array: ARRAYED_COLLECTION[REAL]
		do
			array?=some_reals
			if array=Void then 
				create {FAST_ARRAY[REAL]} array.from_collection(some_reals)
			end
	...
	set_comment (a_group, a_key, a_comment: STRING) is
			...
			-- `is_successful' will be True if `a_comment' has actually been
			-- set.
		require comment_not_void: a_comment/=Void
		do
			is_successful := g_key_file_set_comment
			(handle, null_or_string(a_group),
