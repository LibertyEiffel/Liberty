...
feature {ANY} -- Adding:
   ...
   add_last (element: like item) is
      ...
      do
         ...
         check
            count = upper + 1
         end -- because the next put command relies on this assumption.
...
feature {ANY} -- TODO: unimplemented
   ...
   fast_has (an_item: ITEM): BOOLEAN is
      do
         not_yet_implemented
      end

   has (an_item: ITEM): BOOLEAN is
      do
         not_yet_implemented
      end
   ...
   append_collection (another: COLLECTION[ITEM]) is
      do
         not_yet_implemented
      end
