indexing
   description:
      "A string made from a `C' const char pointer."
   copyright:
      "[
...
class CONST_STRING
obsolete once "Shall be adapted for Liberty Eiffel"
...
inherit
   ABSTRACT_STRING
      ...
      redefine resize, clear_count, wipe_out, clear_count_and_capacity, fill_with, replace_all, append, append_string, append_substring,
         infix "+", infix "<", infix ">", infix "<=", infix ">=", compare, three_way_comparison, as_upper, as_lower, prepend,
         insert_string, replace_substring, put, swap, insert_character, shrink, remove, add_first, precede, add_last, append_character,
         extend, to_lower, to_upper, remove_substring, remove_between, remove_suffix, remove_prefix, left_adjust, right_adjust,
         do_all, extend_multiple, precede_multiple, extend_to_count, precede_to_count, reverse, remove_all_occurrences,
         extend_unless, from_external
      end
...
feature {ANY}
   ...
   modify is
      ...
      local
         previous: like storage; i: INTEGER
      do
...
feature {ANY}
   -- commented out to achieve compatibility with both SE 2.2 and 2.3
...
feature {}
   -- functions from STRING that change signature in 2.3. Note: commented out to achieve compatibility with both SE 2.2 and 2.3
...
feature {} -- Implementation
   ...
   dispose_notice: STRING is "CONST_STRING.dispose: the string is unchanged; using a tentative hack to avoid crash during quitting or disposing; a pre-allocated 1-char-long memory area will be set as storage.%N"
