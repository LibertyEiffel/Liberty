-- This file have been created by wrapper-generator.
-- Any change will be lost by the next execution of the tool.

deferred class GIBASE_INFO_STUB_STRUCT

insert STANDARD_C_LIBRARY_TYPES

	GOBJECT_INTROSPECTION_TYPES
feature {} -- Low-level setters

	gibase_info_stub_struct_set_dummy1 (a_structure: POINTER; a_value: INTEGER_32) is
			-- Setter for dummy1 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy1"
		}"
		end

	gibase_info_stub_struct_set_dummy2 (a_structure: POINTER; a_value: INTEGER_32) is
			-- Setter for dummy2 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy2"
		}"
		end

	gibase_info_stub_struct_set_dummy3 (a_structure: POINTER; a_value: POINTER) is
			-- Setter for dummy3 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy3"
		}"
		end

	gibase_info_stub_struct_set_dummy4 (a_structure: POINTER; a_value: POINTER) is
			-- Setter for dummy4 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy4"
		}"
		end

	gibase_info_stub_struct_set_dummy5 (a_structure: POINTER; a_value: POINTER) is
			-- Setter for dummy5 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy5"
		}"
		end

	gibase_info_stub_struct_set_dummy6 (a_structure: POINTER; a_value: NATURAL_32) is
			-- Setter for dummy6 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy6"
		}"
		end

	gibase_info_stub_struct_set_dummy7 (a_structure: POINTER; a_value: NATURAL_32) is
			-- Setter for dummy7 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_dummy7"
		}"
		end

	gibase_info_stub_struct_set_padding (a_structure: POINTER; a_value: POINTER) is
			-- Setter for padding field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: setter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_set_padding"
		}"
		end

feature {} -- Low-level queries

	gibase_info_stub_struct_get_dummy1 (a_structure: POINTER): INTEGER_32 is
			-- Query for dummy1 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy1"
		}"
		end

	gibase_info_stub_struct_get_dummy2 (a_structure: POINTER): INTEGER_32 is
			-- Query for dummy2 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy2"
		}"
		end

	gibase_info_stub_struct_get_dummy3 (a_structure: POINTER): POINTER is
			-- Query for dummy3 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy3"
		}"
		end

	gibase_info_stub_struct_get_dummy4 (a_structure: POINTER): POINTER is
			-- Query for dummy4 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy4"
		}"
		end

	gibase_info_stub_struct_get_dummy5 (a_structure: POINTER): POINTER is
			-- Query for dummy5 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy5"
		}"
		end

	gibase_info_stub_struct_get_dummy6 (a_structure: POINTER): NATURAL_32 is
			-- Query for dummy6 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy6"
		}"
		end

	gibase_info_stub_struct_get_dummy7 (a_structure: POINTER): NATURAL_32 is
			-- Query for dummy7 field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_dummy7"
		}"
		end

	gibase_info_stub_struct_get_padding (a_structure: POINTER): POINTER is
			-- Query for padding field of GIBASE_INFO_STUB_STRUCT structure.
			-- TODO: getter description

		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "gibase_info_stub_struct_get_padding"
		}"
		end

feature -- Structure size
	struct_size: like size_t is
		external "plug_in"
		alias "{
			location: "."
			module_name: "plugin"
			feature_name: "sizeof__GIBaseInfoStub"
		}"
		end

end -- class GIBASE_INFO_STUB_STRUCT
-- This file have been created by wrapper-generator.
-- Any change will be lost by the next execution of the tool.

