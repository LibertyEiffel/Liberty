...
feature {} -- External calls
	...
	cairo_append_path (a_cr, a_path: POINTER)
			-- 	void cairo_append_path (cairo_t *cr, cairo_path_t *path);
		external "C use <cairo.h>"
		end

	cairo_get_current_point (a_cr, an_x, an_y: POINTER)
	...
	cairo_arc (a_cr: POINTER; an_xc, an_yc, a_radius, an_angle1, an_angle2: REAL)
			-- void cairo_arc (cairo_t *cr, double xc, double yc, double
			-- radius, double angle1, double angle2);
		external "C use <cairo.h>"
		end

	cairo_arc_negative (a_cr: POINTER; an_xc, an_yc, a_radius, an_angle1, an_angle2: REAL)
			-- void cairo_arc_negative (cairo_t *cr, double xc, double
			-- yc, double radius, double angle1, double angle2);
		external "C use <cairo.h>"
		end

	cairo_curve_to (a_cr: POINTER; an_x1, a_y1, a_x2, a_y2, a_x3, an_y3: REAL)
			-- void cairo_curve_to (cairo_t *cr, double x1, double y1,
			-- double x2, double y2, double x3, double y3);
		external "C use <cairo.h>"
		end

	cairo_line_to (a_cr: POINTER; an_x, an_y: REAL)
			-- 	void cairo_line_to (cairo_t *cr, double x, double y);
		external "C use <cairo.h>"
		end

	cairo_move_to (a_cr: POINTER; an_x, an_y: REAL)
			-- void cairo_move_to (cairo_t *cr, double x, double y);
		external "C use <cairo.h>"
		end

	cairo_rectangle (a_cr: POINTER; an_x, an_y, a_width, an_height: REAL)
			-- void cairo_rectangle (cairo_t *cr, double x, double y,
			-- double width, double height);
		external "C use <cairo.h>"
		end

	cairo_glyph_path (a_cr, some_glyphs: POINTER; a_num_glyphs: INTEGER)
			-- 	void cairo_glyph_path (cairo_t *cr, cairo_glyph_t *glyphs, int num_glyphs);
		external "C use <cairo.h>"
		end

	cairo_text_path (a_cr, an_utf8: POINTER)
			-- 	void cairo_text_path (cairo_t *cr, const char *utf8);
		external "C use <cairo.h>"
		end

	cairo_rel_curve_to (a_cr: POINTER; a_dx1, a_dy1, a_dx2, a_dy2, a_dx3, a_dy3: REAL)
			--	void cairo_rel_curve_to (cairo_t *cr, double dx1, double
			--	dy1, double dx2, double dy2, double dx3, double dy3);
		external "C use <cairo.h>"
		end

	cairo_rel_line_to (a_cr: POINTER; a_dx, a_dy: REAL)
			-- 	void cairo_rel_line_to (cairo_t *cr, double dx, double dy);
		external "C use <cairo.h>"
		end

	cairo_rel_move_to (a_cr: POINTER; a_dx, a_dy: REAL)
			-- void cairo_rel_move_to (cairo_t *cr, double dx, double dy);
		external "C use <cairo.h>"
		end

	cairo_path_get_status (a_path: POINTER): INTEGER
