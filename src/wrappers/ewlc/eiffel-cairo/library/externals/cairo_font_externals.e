...
feature {} -- External calls
	cairo_select_font_face (a_context, a_family: POINTER; a_slant, a_weight: INTEGER) is
	...
	cairo_set_font_size (a_context: POINTER; a_size: REAL) is
			-- void cairo_set_font_size (cairo_t *cr, double size);
		external "C use <cairo.h>"
		end

	cairo_set_font_matrix (a_context, a_matrix: POINTER) is
			-- void cairo_set_font_matrix (cairo_t *cr, const cairo_matrix_t *matrix);
		external "C use <cairo.h>"
		end

	cairo_get_font_matrix (a_context, a_matrix: POINTER) is
			-- void cairo_get_font_matrix (cairo_t *cr, cairo_matrix_t
			-- *matrix);
		external "C use <cairo.h>"
		end

	cairo_set_font_options (a_context, a_font_options: POINTER) is
			-- void cairo_set_font_options (cairo_t *cr, const
			-- cairo_font_options_t *options);
		external "C use <cairo.h>"
		end

	cairo_get_font_options (a_context, a_font_options: POINTER) is
			-- void cairo_get_font_options (cairo_t *cr,
			-- cairo_font_options_t *options);
		external "C use <cairo.h>"
		end

	cairo_show_text (a_context, an_utf8: POINTER) is
			-- void cairo_show_text (cairo_t *cr, const char *utf8);
		external "C use <cairo.h>"
		end

	cairo_show_glyphs (a_context, some_glyphs: POINTER; an_n_glyphs: INTEGER) is
			-- void cairo_show_glyphs (cairo_t *cr, cairo_glyph_t
			-- *glyphs, int num_glyphs);
		external "C use <cairo.h>"
		end

	cairo_get_font_face (a_context: POINTER): POINTER is
			-- cairo_font_face_t* cairo_get_font_face (cairo_t *cr);
		external "C use <cairo.h>"
		end

	cairo_font_extents (a_context, an_extents: POINTER) is
			-- void cairo_font_extents (cairo_t *cr, cairo_font_extents_t
			-- *extents);
		external "C use <cairo.h>"
		end
	
	cairo_set_font_face (a_context, a_font_face: POINTER) is
			-- void cairo_set_font_face (cairo_t *cr, cairo_font_face_t
			-- *font_face);
		external "C use <cairo.h>"
		end
	
	cairo_set_scaled_font (a_context, a_scaled_font: POINTER) is
			-- void cairo_set_scaled_font (cairo_t *cr, const cairo_scaled_font_t *scaled_font);
		external "C use <cairo.h>"
		end

	cairo_text_extents (a_context, an_utf8, an_extents: POINTER) is
			-- void cairo_text_extents (cairo_t *cr, const char *utf8,
			-- cairo_text_extents_t *extents);
		external "C use <cairo.h>"
		end
	
	cairo_glyph_extents (a_context, some_glyphs: POINTER; an_n_glyphs: INTEGER; an_extents: POINTER) is
