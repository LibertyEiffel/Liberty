...
feature {} -- External calls
	...
	g_completion_new (a_gcompletionfunc:POINTER): POINTER is
			-- 	GCompletion* g_completion_new (GCompletionFunc func);
		external "C use <glib.h>"
		end

	g_completion_add_items (a_cmp, some_items: POINTER) is
			--  void g_completion_add_items (GCompletion *cmp, GList *items);
		external "C use <glib.h>"
		end
	
	g_completion_remove_items (a_cmp, some_items: POINTER) is
	...
	g_completion_complete (a_cmp, a_prefix, a_new_prefix_ref: POINTER): POINTER is
			--  GList* g_completion_complete (GCompletion *cmp, const gchar *prefix, gchar **new_prefix);
		external "C use <glib.h>"
		end
		
	g_completion_complete_utf8 (a_cmp, a_prefix, a_new_prefix_ref: POINTER): POINTER is
			--  GList* g_completion_complete_utf8 (GCompletion *cmp, const gchar *prefix, gchar **new_prefix);
		external "C use <glib.h>"
		end
	
	g_completion_set_compare (a_cmp, a_gcompletion_strncmp: POINTER) is
