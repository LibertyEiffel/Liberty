note
	description: "GtkTreeModelSort -- a GtkTreeModel which makes an underlying tree model sortable."
	copyright: "[
...
class GTK_TREE_MODEL_SORT
	-- The GtkTreeModelSort is a model which implements the
	-- GtkTreeSortable interface. It does not hold any data itself, but
	-- rather is created with a child model and proxies its data. It
	-- has identical column types to this child model, and the changes
	-- in the child are propagated. The primary purpose of this model
	-- is to provide a way to sort a different model without modifying
	-- it. Note that the sort function used by GtkTreeModelSort is not
	...
	-- GtkTreeModelSort, the two GtkTreeViews can each sort their view
	-- of the data without affecting the other. By contrast, if we
	-- simply put the same model in each widget, then sorting the first
	-- would sort the second.

	-- (TODO: Eiffelize) Example 3. Using a GtkTreeModelSort
	...
	-- find the same selected row on the child model, and change the
	-- row there.

	-- (TODO: Eiffelize) Example 4. Accessing the child model of in a selection changed callback
