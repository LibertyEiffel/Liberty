...
feature
	...
	set_primary_key_fields (some_fields: G_OBJECT_SLIST[GDA_DICT_FIELD]) is
	...
	-- foreign_key_fields: G_OBJECT_SLIST[GDA_DICT_CONSTRAINT_FOREIGN_KEY_PAIR] is
	...
	set_foreign_key_actions (on_update, on_delete: INTEGER) is
			...
			-- TODO: require valid_update_action:
			-- is_valid_foreign_key_action (on_update)
			-- valid_delete_action: is_valid_foreign_key_action
			-- (on_delete)
		do
	...
	foreing_key_actions: TUPLE[INTEGER,INTEGER] is
		...
		local on_update, on_delete: INTEGER
		do
			gda_dict_constraint_fkey_get_actions(handle,$on_update,$on_delete)
			create Result.make_2(on_update,on_delete)
		end

	set_unique_fields (some_fields: G_OBJECT_SLIST[GDA_DICT_FIELD]) is
