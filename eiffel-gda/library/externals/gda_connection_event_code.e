indexing
	description: "Enum GdaConnectionEventCode"
	status: "[
                  AUTOMATICALLY GENERATED FILE. 

                  ANY CHANGE TO THIS WILL BE OVERWRITTEN BY NEXT
                  EXECUTION OF GENERATING SCRIPT!  You can put your
                  changes in the patch file gda_connection_event_code.e.patch;
...
feature -- Queries
    ...
    is_constraint_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_constraint_violation) end

    is_restrict_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_restrict_violation) end

    is_not_null_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_not_null_violation) end

    is_foreign_key_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_foreign_key_violation) end

    is_unique_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_unique_violation) end

    is_check_violation: BOOLEAN is do Result:=(value=gda_connection_event_code_check_violation) end

    is_insufficient_privileges: BOOLEAN is do Result:=(value=gda_connection_event_code_insufficient_privileges) end

    is_undefined_column: BOOLEAN is do Result:=(value=gda_connection_event_code_undefined_column) end

    is_undefined_function: BOOLEAN is do Result:=(value=gda_connection_event_code_undefined_function) end

    is_undefined_table: BOOLEAN is do Result:=(value=gda_connection_event_code_undefined_table) end

    is_duplicate_column: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_column) end

    is_duplicate_database: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_database) end

    is_duplicate_function: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_function) end

    is_duplicate_schema: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_schema) end

    is_duplicate_table: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_table) end

    is_duplicate_alias: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_alias) end

    is_duplicate_object: BOOLEAN is do Result:=(value=gda_connection_event_code_duplicate_object) end

    is_syntax_error: BOOLEAN is do Result:=(value=gda_connection_event_code_syntax_error) end

    is_unknown: BOOLEAN is do Result:=(value=gda_connection_event_code_unknown) end


    is_valid_value (a_value: INTEGER): BOOLEAN is 
        do 
           Result:=(
                    (a_value=gda_connection_event_code_constraint_violation) or else
                    ...
                    (a_value=gda_connection_event_code_unknown) or else
...
end -- class GDA_CONNECTION_EVENT_CODE
