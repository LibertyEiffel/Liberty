...
feature -- Queries
    ...
    is_xml_load_error: BOOLEAN is do Result:=(value=gda_query_field_field_xml_load_error) end

    is_render_error: BOOLEAN is do Result:=(value=gda_query_field_field_render_error) end


    is_valid_value (a_value: INTEGER): BOOLEAN is 
        do 
           Result:=(
                    (a_value=gda_query_field_field_xml_load_error) or else
                    (a_value=gda_query_field_field_render_error) or else
...
feature {WRAPPER, WRAPPER_HANDLER} -- Low level values
    ...
    gda_query_field_field_xml_load_error: INTEGER is
         external "C macro use /usr/include/libgda-3.0/libgda/gda-query-field-field.h"
...
end -- class GDA_QUERY_FIELD_FIELD_ERROR
