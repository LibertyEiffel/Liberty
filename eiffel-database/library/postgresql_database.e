...
--            blocked on remote I/O whilst doing so. The point of this approach
--            is that the waits for I/O to complete can occur in the
--            application's main loop, rather than down inside PQconnectdb, and
--            so the application can manage this operation in parallel with
--            other activities.
...
--            Neither PQconnectStart nor PQconnectPoll will block, so long as a
--            number of restrictions are met:
...
--            free the memory used by the PGconn object. The PGconn pointer must
--            not be used again after PQfinish has been called.
