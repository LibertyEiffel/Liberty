...
deferred class GLIB_MAIN_EVENT_LOOP
	-- The Main Event Loop manages all available sources of events.
	
	-- The main event loop manages all the available sources of events
	-- for GLib and GTK+ applications. These events can come from any
	-- number of different types of sources such as file descriptors
	-- (plain files, pipes or sockets) and timeouts. New types of event
	-- sources can also be added using (TODO) `g_source_attach'.

	-- To allow multiple independent sets of sources to be handled in
	-- different threads, each source is associated with a
	-- G_MAIN_CONTEXT. A G_MAIN_CONTEXT can only be running in a single
	-- thread, but sources can be added to it and removed from it from
	-- other threads.

	-- Each event source is assigned a priority. The default priority,
	-- `g_priority_default,' is 0. Values less than 0 denote higher
	-- priorities. Values greater than 0 denote lower
	-- priorities. Events from high priority sources are always
	-- processed before events from lower priority sources.

	-- Idle functions can also be added, and assigned a priority. These
	-- will be run whenever no events with a higher priority are ready
	-- to be processed.
	
	-- The G_MAIN_LOOP data type represents a main event loop. A
	-- G_MAIN_LOOP is created with `make'. After adding the initial
	-- event sources, `run' is called. This continuously checks for new
	-- events from each of the event sources and dispatches
	-- them. Finally, the processing of an event from one of the
	-- sources leads to a call to `quit' to exit the main loop, and
	-- `run' returns.

	-- It is possible to create new instances of GMainLoop
	-- recursively. This is often used in GTK+ applications when
	-- showing modal dialog boxes. Note that event sources are
	-- associated with a particular GMainContext, and will be checked
	-- and dispatched for all main loops associated with that
	-- GMainContext.

	-- GTK+ contains wrappers of some of these functions,
	-- e.g. gtk_main(), gtk_main_quit() and gtk_events_pending().
	...
	-- created by deriving from the GSource structure. The derived type
	-- of source is represented by a structure that has the GSource
	-- structure as a first element, and other elements specific to the
	-- new source type. To create an instance of the new source type,
	-- call g_source_new() passing in the size of the derived structure
	-- and a table of functions. These GSourceFuncs determine the
	-- behavior of the new source types.

	-- New source types basically interact with with the main context
	-- in two ways. Their prepare function in GSourceFuncs can set a
	-- timeout to determine the maximum amount of time that the main
	-- loop will sleep before checking the source again. In addition,
	-- or as well, the source can add file descriptors to the set that
	-- the main context checks using g_source_add_poll().
	...
	-- instance, when integrating the GMainLoop with an external main
	-- loop. In such cases, you can call the component functions of
	-- `iteration' directly. These functions are `prepare', `query',
	-- `check_context'() and `dispatch'.

	-- The operation of these functions can best be seen in terms of a
	-- state diagram, as shown in Figure 1, `States of a Main Context'.
