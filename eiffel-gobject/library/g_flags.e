...
deferred class G_FLAGS
	-- The flags type of GLib type system. Flags are like enumerations,
	-- but allow their values to be combined by bitwise or. A
	-- registered enumeration or flags type associates a name and a
	-- nickname with each allowed value, and the methods
	-- `value_by_name' and `value_by_nick' can look up values by their
	-- name or nickname. When a flags type is registered with the GLib
	-- type system, it can be used as value type for object properties,
	-- using `flags_property' or `set_flags_property' (TODO).
	
	-- GObject ships with a utility called glib-mkenums that can
	-- construct suitable type registration functions from C
	-- enumeration definitions. TODO: provide the equivalent Eiffel tool
inherit
	SHARED_C_STRUCT
insert
	G_FLAGS_EXTERNALS
