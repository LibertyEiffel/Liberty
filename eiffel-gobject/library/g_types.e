...
feature {}
	-- numerical values of fundamental GTypes shall be listed in
	-- G_TYPE_EXTERNALS
feature
	...
	are_valid_gtypes (some_types: COLLECTION[INTEGER_32]): BOOLEAN is
			-- Are all types valid g_types?

			-- TODO: when NATURAL_32 is available `some_types' will become COLLECTION[NATURAL_32].
		require some_types_not_void: some_types /= Void
		-- local i: INTEGER
		do
			-- from Result:=True; i := some_types.lower
			-- until not Result or else i > some_types.upper
			-- loop
			--	Result := is_g_type(some_types.item(i).to_integer_64)
			--	i := i + 1
			-- end
			-- Working implementation when some_types will be a
