...
feature {ANY}
   make is
      do
         create pam_pro.make
         create pam_printer.make
         create pam_parser.make
         -- test1;
         -- test2;
         -- test3;
         -- test4;
   ...
   parse (txt: STRING) is
      do
         ...
         if pam_parser.error_detected or else pam_parser.warning_detected or else pam_parser.query then
   ...
   query (txt: STRING) is
      do
         pam_parser.parse(txt)
         if pam_parser.error_detected or else pam_parser.warning_detected or else pam_parser.fact then
            print(pam_parser.error_message)
            die_with_code(exit_failure_code)
         end
