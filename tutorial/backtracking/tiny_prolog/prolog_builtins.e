...
feature {ANY}
   evaluate_current_state is
      local
         atom: PROLOG_ATOM; compound: PROLOG_COMPOUND; functor: PROLOG_FUNCTOR
      do
...
feature {}
   init_builtins is
      do
         if builtins.is_empty then
            really_init_builtins
         end
      end
...
feature {} -- type testing
   bi_atom is
      do
         inspect
            current_compound.args.first.type
         when type_atom then
            partial_success
         else
            partial_fail
   ...
   bi_number is
      do
         inspect
            current_compound.args.first.type
         when type_integer, type_float then
            partial_success
         else
            partial_fail
         end
   ...
   bi_atomic is
      do
         inspect
            current_compound.args.first.type
         when type_atom, type_integer, type_float then
            partial_success
         else
            partial_fail
         end
   ...
   bi_callable is
      do
         inspect
            current_compound.args.first.type
         when type_atom, type_compound then
            partial_success
         else
            partial_fail
         end
