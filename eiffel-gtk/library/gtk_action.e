indexing
	...
	copyright: "[
					...
					-- Actions represent operations that the user can be
					-- perform, along with some information how it should
					-- be presented in the interface. Each action provides
					-- methods to create icons, menu items and toolbar
					-- items representing itself.
											
					-- As well as the callback that is called when the
					-- action gets activated, the following also gets
					...
					-- radio actions, of which only one in a group can be
					-- in the "active" state. Other actions can be
					-- implemented as GtkAction subclasses.

					-- Each action can have one or more proxy menu item,
					-- toolbar button or other proxy widgets. Proxies
					-- mirror the state of the action (text label, tooltip,
					-- icon, visible, sensitive, etc), and should change
					-- when the action's state changes. When the proxy is
