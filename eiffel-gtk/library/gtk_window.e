...
insert
	...
	GTK_WINDOW_EXTERNALS -- redefine fill_tagged_out_memory end
	GTK_WINDOW_TYPE -- redefine fill_tagged_out_memory end 

creation dummy, from_external_pointer, make, make_toplevel, make_popup
...
feature
	...
	make_popup is
		do
			from_external_pointer (gtk_window_new (gtk_window_popup))
		end
...
feature -- Focusing
	...
	title: STRING is
		...
		local ptr: POINTER
		do
			ptr:=gtk_window_get_title (handle)
			if ptr.is_not_null then
...
feature
	...
	-- 		do
	-- 			t:=title
	-- 			if t=Void then tagged_out_memory.append(once "no title")
	-- 			else
	-- 				tagged_out_memory.append(once "title :'")
