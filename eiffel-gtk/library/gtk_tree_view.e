...
feature
	model: GTK_TREE_MODEL is
		...
		do
			...
			if ptr.is_not_null then create Result.from_pointer (ptr) end
		end
...
feature 
	selection: GTK_TREE_SELECTION is
		...
		do
			create Result.from_pointer (gtk_tree_view_get_selection (handle))
		end
	...
	append_column (a_column: GTK_TREE_VIEW_COLUMN) is
		...
		require
			...
			fixed_height_requirements: is_height_fixed implies a_column.is_sizing_fixed
		do
	...
	column (a_position: INTEGER): GTK_TREE_VIEW_COLUMN is
		...
		require valid_number: a_number.in_range(0,columns.count)
		local ptr: POINTER
		do
			create Result.from_pointer (gtk_tree_view_get_column (handle, a_number))
		end

	columns: G_LIST[GTK_TREE_VIEW_COLUMN] is
			-- a GList of all the GtkTreeViewColumn s currently in tree view.
		do
			create Result.from_pointer (gtk_tree_view_get_columns(handle))
			-- Note: The returned list must be freed with g_list_free().
		end

	
	move_column_after (a_column, a_base: GTK_TREE_VIEW_COLUMNS) is
			-- Moves `a_column' to be after to `a_base' column. If
			-- `a_base' is Void, then column is placed in the first
			-- position.
		require valid_column: a_colunmn/=Void
		do
			if a_base /= Void then 
	...
	set_expander_column (a_column: GTK_TREE_VIEW_COLUMNS) is
	...
	expander_column: GTK_TREE_VIEW_COLUMN is
		...
		do
			create Result.from_pointer (gtk_tree_view_get_expander_column (handle))
		end
