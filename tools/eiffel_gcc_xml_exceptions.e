	...
	exception_label (an_exception_number: INTEGER): STRING is
		do
			...
			when Check_instruction then
				Result := check_instruction_exception
			when Class_invariant then
				Result := class_invariant_exception
			when Developer_exception then
				Result := developer_exception_exception
			when Incorrect_inspect_value then
				Result := incorrect_inspect_value_exception
			when Loop_invariant then
				Result := loop_invariant_exception
			when Loop_variant then
				Result := loop_variant_exception
			when No_more_memory then
				Result := no_more_memory_exception
			when Postcondition then
				Result := postcondition_exception
			when Precondition then
				Result := precondition_exception
			when Routine_failure then
				Result := routine_failure_exception
			when Os_signal then
				Result := os_signal_exception
			when Void_attached_to_expanded then
				Result := void_attached_to_expanded_exception
			when Void_call_target then
				Result := void_call_target_exception
			when System_level_type_error then
				Result := system_level_type_error_exception
			else
